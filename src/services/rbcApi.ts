// RBC InvestEase API Integration
...
class RBCApiService {
  ...
  async registerTeam(teamData: TeamRegistration): Promise<TeamAuthResponse> {
    ...
    localStorage.setItem('rbc_token', data.jwtToken);
    localStorage.setItem('rbc_teamId', data.teamId);
  ...
  initializeFromStorage(): boolean {
    ...
    const teamId = localStorage.getItem('rbc_teamId');
  ...
  async createPortfolio(clientId: string, portfolioData: { type: string; initialAmount: number }): Promise<Portfolio> {
    const response = await fetch(`${API_BASE_URL}/clients/${clientId}/portfolios`, {
      method: 'POST',
      headers: this.getAuthHeaders(),
  ...
  async transferToPortfolio(portfolioId: string, amount: number): Promise<{ portfolio: Portfolio; client_cash: number }> {
    const response = await fetch(`${API_BASE_URL}/portfolios/${portfolioId}/transfer`, {
      method: 'POST',
      headers: this.getAuthHeaders(),
  ...
  async withdrawFromPortfolio(portfolioId: string, amount: number): Promise<{ portfolio: Portfolio; client_cash: number }> {
    const response = await fetch(`${API_BASE_URL}/portfolios/${portfolioId}/withdraw`, {
      method: 'POST',
      headers: this.getAuthHeaders(),
      body: JSON.stringify({ amount }),
    });
  ...
  async simulateClient(clientId: string, months: number): Promise<{ results: SimulationResult[] }> {
    const response = await fetch(`${API_BASE_URL}/client/${clientId}/simulate`, {
      method: 'POST',
      headers: this.getAuthHeaders(),
  ...
  async getPortfolioAnalysis(portfolioId: string): Promise<{
    ...
    const response = await fetch(`${API_BASE_URL}/portfolios/${portfolioId}/analysis`, {
      method: 'GET',
      headers: this.getAuthHeaders(),
    });
  ...
  clearAuth(): void {
    ...
    localStorage.removeItem('rbc_teamId');
  }
}
