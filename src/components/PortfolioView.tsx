    ...
    <div className="min-h-screen text-white p-6 flex flex-col scanlines pixel-font" style={{backgroundColor: '#061625'}}>
      ...
      >
        <div className="border-4 neon-border-blue p-12 w-full max-w-2xl" style={{backgroundColor: 'rgba(6, 22, 37, 0.8)'}}>
          <div className="mb-6">
            <div className="text-lg text-[#00e1ff] mb-2">TOTAL PORTFOLIO VALUE</div>
            <motion.div
              animate={{ 
                scale: [1, 1.02, 1],
                textShadow: [
              ...
              className="text-8xl text-[#fff900] mb-4"
            >
              ${totalValue.toFixed(2)}
            </motion.div>
            <div className={`text-3xl ${gainLoss >= 0 ? 'text-[#39ff14]' : 'text-[#ff0000]'}`}>
          ...
          <div className="grid grid-cols-2 gap-8 mb-8 mt-[3rem]">
            <div className="text-center border-2 neon-border-blue p-4" style={{backgroundColor: 'rgba(0, 225, 255, 0.1)'}}>
              <div className="text-lg text-[#00e1ff] mb-2">💰 CASH</div>
              <div className="text-3xl text-[#00e1ff]">${cash.toFixed(2)}</div>
            ...
            <div className="text-center border-2 neon-border-pink p-4" style={{backgroundColor: 'rgba(255, 0, 233, 0.1)'}}>
              <div className="text-lg text-[#ff00e9] mb-2">📈 INVESTMENTS</div>
              <div className="text-3xl text-[#ff00e9]">${portfolioValue.toFixed(2)}</div>
          ...
          {portfolioEntries.length > 0 && (
            <div className="mb-6">
              <div className="text-lg text-[#6100ff] mb-4">🏢 HOLDINGS</div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          <div className="text-[#6100ff]">{symbol}</div>
                          <div className="text-sm text-white">{shares} shares</div>
                        </div>
                        <div className="text-right">
                          <div className="text-[#fff900]">${value.toFixed(2)}</div>
                          <div className={`text-sm ${stock.change >= 0 ? 'text-[#39ff14]' : 'text-[#ff0000]'}`}>
          ...
          <div className="mb-6">
            <div className="text-lg text-[#39ff14] mb-4">📈 MARKET PRICES</div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {stocks.map((stock) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="text-[#39ff14]">{stock.symbol}</div>
                      <div className="text-sm text-white">{stock.name}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-[#fff900]">${stock.price.toFixed(2)}</div>
                      <div className={`text-sm ${stock.change >= 0 ? 'text-[#39ff14]' : 'text-[#ff0000]'}`}>
          ...
          {portfolioEntries.length === 0 && !isAITurn && (
            <div className="text-center text-white mb-1 border-2 neon-border-purple p-6" style={{backgroundColor: 'rgba(97, 0, 255, 0.1)'}}>
              <div className="text-4xl mb-2">📊</div>
              <div className="text-[#6100ff]">NO INVESTMENTS YET</div>
          ...
          {isAITurn && (
            <div className="text-center text-white mb-6 border-2 neon-border-cyan p-6" style={{backgroundColor: 'rgba(0, 255, 255, 0.1)'}}>
              <div className="text-4xl mb-2">🤖</div>
              <div className="text-[#00e1ff]">AI IS ANALYZING MARKET AND EXECUTING TRADES</div>
      ...
      {isAITurn && (
        <motion.div
          ...
          className="text-center"
        >
          <div className="text-2xl text-[#00e1ff] neon-text pixel-font">🤖 AI EXECUTING TRADES...</div>
        </motion.div>
      )}
    </div>
  );
