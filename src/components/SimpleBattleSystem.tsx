...
interface SimpleBattleSystemProps {
  gameConfig: GameConfig;
  onBattleComplete: (results: RoundResult[]) => void;
}

export function SimpleBattleSystem({ gameConfig, onBattleComplete }: SimpleBattleSystemProps) {
  const [currentRound, setCurrentRound] = useState(1);
  const [battlePhase, setBattlePhase] = useState('setup' as 'setup' | 'player' | 'vs' | 'ai' | 'results' | 'complete');
